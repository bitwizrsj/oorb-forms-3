  ...
  const checkAuthStatus = async () => {
    ...
    try {
      ...
      if (token) {
        try {
          if (storedUser) {
            // Parse stored user first (optional speedup)
            const userData = JSON.parse(storedUser);
            console.log('Auth Context: Found stored user:', userData.name, userData.email);
          }

          // Verify token is still valid (and fetch user if storedUser was missing!)
        ...
        console.log('Auth Context: No valid auth token found');
        setUser(null);
      }
    } catch (error) {
